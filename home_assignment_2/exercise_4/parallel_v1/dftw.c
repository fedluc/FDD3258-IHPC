	...
	// idft: 1 direct DFT, -1 inverse IDFT (Inverse DFT)
	  ...
	  // Split the calculation of the dft over different threads
          for (int k=0 ; k<N ; k++) {
		...
		// Variable used to store the partial result
		double Xr_o_partial = 0.0, Xi_o_partial = 0.0;

		// Compute partial result
                #pragma omp for 
                  for (int n=0 ; n<N ; n++)  {
                    // Real part of X[k]
                    Xr_o_partial += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
                    // Imaginary part of X[k]
                    Xi_o_partial += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
                  }
